...
class SimulationPageAnim(w: Int, h: Int, view: View, listener: PageAnimation.OnPageChangeListener) :
    ...
    private var mBackShadowDrawableLR: GradientDrawable // 有阴影的GradientDrawable
    ...
    //上一页滑动不出现对角
    override var direction: PageAnimation.Direction
        get() = super.direction
        set(direction) {
            super.direction = direction
            when (direction) {
                PageAnimation.Direction.PRE -> if (mStartX > mScreenWidth / 2) {
    ...
    override fun drawMove(canvas: Canvas) {
        when (direction) {
            PageAnimation.Direction.NEXT -> {
                ...
                calcPoints()
                drawCurrentPageArea(canvas, nextBitmap!!, mPath0)
    ...
    override fun startAnim() {
        ...
        if (isCancel) {
            ...
            dx = if (mCornerX > 0 && direction == PageAnimation.Direction.NEXT) {
    ...
    override fun setTouchPoint(x: Float, y: Float) {
        ...
        //触摸y中间位置吧y变成屏幕高度
        if (mStartY > mScreenHeight / 3 && mStartY < mScreenHeight * 2 / 3 || direction == PageAnimation.Direction.PRE) {
            mTouchY = mScreenHeight.toFloat()
        }

        if (mStartY > mScreenHeight / 3 && mStartY < mScreenHeight / 2 && direction == PageAnimation.Direction.NEXT) {
            mTouchY = 1f
        }
    }
